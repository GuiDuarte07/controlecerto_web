...
import { useTicketDetail } from "../hooks/useTickets.hooks";
import { resolveTicketStatusKey } from "../types/tickets.types";
...
interface TicketDetailPageContentProps {
  ticketId: number;
}

export function TicketDetailPageContent({ ticketId }: TicketDetailPageContentProps) {
  ...
  const {
    ...
    closeTicket,
    reopenTicket,
  } = useTicketDetail(ticketId, true);

  const statusKey = ticket ? resolveTicketStatusKey(ticket.status) : "OPEN";
  const canClose = ticket && statusKey !== "CLOSED";
  const canReopen = ticket && statusKey === "CLOSED";
  ...
  usePageHeader({
    title: ticket ? t("detail.title", { id: String(ticket.id) }) : t("detail.title", { id: "" }),
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <div className="text-xs font-medium">
                        {isAdmin ? t("detail.authorAdmin") : t("detail.authorYou")}
                      </div>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  toast.success(t("feedback.reopened"));
                } catch {
                  toast.error(t("feedback.updateStatusError"));
                }
              }}
